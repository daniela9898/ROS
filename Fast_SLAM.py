...
# Not working yet, should be used for mapping the robot position and environment
# Fast SLAM using Particle Filter and Extended Kalman Filter
# Needs orientation-aware-sensor-model_euclidean.py to calculare the euclidean distance to mannequins and walls
...
class SLAMParticleFilter:
    ...
    def particle_filter(self, particle_set, u, y, siga=0.03, sigb=0.15):
        ...
        for k in range(particle_set['mu_robot'].shape[0]):
            ...
            # measurement update
            for s in range(particle_set['mu_beacons'].shape[1]):
                # Compute the weight
                mu_bar = particle_set['mu_beacons'][k][s]
                y_bar = mu_bar - x_bar[:2]
                y_dev = y_bar - y[s * 2:(s * 2) + 2]
                Q = np.identity(2) * sigb ** 2
                LocSig = particle_set['Sigma_beacons'][k][s] + Q
                ...
                Sigma_bar = particle_set['Sigma_beacons'][k][s]
                C = np.identity(2)
                D = -x_bar[:2]
                # The Kalman formulas
                K = Sigma_bar @ C.T @ np.linalg.inv(C @ Sigma_bar @ C.T + Q)
                mu = mu_bar + K @ (y[s * 2:(s * 2) + 2] - D - C @ mu_bar)
                Sigma = (np.identity(2) - K @ C) @ Sigma_bar

                # Nothing more to do: the result of these two steps is your new particle
                particle_set['mu_beacons'][k][s] = mu
                particle_set['Sigma_beacons'][k][s] = Sigma
            particle_set['mu_robot'][k] = x_bar


        # Normalize just to avoid that all weights go to zero or infinity in the long run
