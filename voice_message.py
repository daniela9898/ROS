...
class produceVoiceMessage:
    ...
    def run(self):
        a=1   # message from the topic which says true if I am near the mannequin and so I need to serve the mannequin
        # Create a new SoundClient where the default behavior *is* to block.

        soundhandle = SoundClient(blocking=True)

        soundhandle.say('Do you want a drink')

        soundhandle.say('Merry christmas', blocking=False)
        ros.sleep(1)
